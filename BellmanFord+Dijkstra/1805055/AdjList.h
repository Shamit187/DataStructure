...
public:
    ...
    bool const operator==(vertexKey &rhs){return key == rhs.key;}
    bool const operator<(vertexKey &rhs){return key > rhs.key;}
    bool const operator>(vertexKey &rhs){return key < rhs.key;}
    vertexKey& operator=(const vertexKey &rhs){
        vertex = rhs.vertex;
        key = rhs.key;
        from = rhs.from;
        return *this;
...
public:
    ...
    std::vector<edge> primMST(unsigned source){
        ...
        vertexKey.resize(nodeNumber, 99999999);
        vertexKey[source] = -1;
    ...
    std::vector<int> BellmanFord(unsigned source, unsigned destination){
        std::vector<double>distance;
        std::vector<int>parent;
        distance.resize(nodeNumber,0);
        parent.resize(nodeNumber, 0);
        ...
        for(int v = 1; v < adjList.size(); v++){
            for(int i = 0; i < adjList.size(); i++){
                for(int j = 0; j < adjList[i].size(); j++){
                    if(distance[adjList[i][j].source] + adjList[i][j].weight < distance[adjList[i][j].destination]){
                        distance[adjList[i][j].destination] = distance[adjList[i][j].source] + adjList[i][j].weight;
                        parent[adjList[i][j].destination] = adjList[i][j].source;
                    }
                }
            }
        }
        ...
        unsigned currentVertex = destination;
        path.push_back(destination);
        while(currentVertex != source){
            currentVertex = parent[currentVertex];
            path.push_back(currentVertex);
        }
        path.push_back(distance[destination]);
        return path;
    }
    std::vector<int> Dijkstra(unsigned source, unsigned destination){
        //init step
        std::vector<double>distance;
        std::vector<int>parent;
        std::vector<bool>visited;
        int visitedNode = 0;
        distance.resize(nodeNumber,INT32_MAX);
        parent.resize(nodeNumber, -1);
        visited.resize(nodeNumber, false);

        //source insertion
        Heap<vertexKey> minQueue;
        distance[source] = 0;
        visited[source] = true;
        for(int i = 0; i < adjList[source].size(); i ++){
            distance[adjList[source][i].destination] = adjList[source][i].weight;
            parent[adjList[source][i].destination] = source;
            minQueue.insert({adjList[source][i].destination, distance[adjList[source][i].destination], 0});
        }

        while(!minQueue.empty()){
            class vertexKey v = minQueue.remove();
            if(visited[v.vertex]) continue;
            visited[v.vertex] = true;
            for(int i = 0; i < adjList[v.vertex].size(); i ++){
                if(distance[adjList[v.vertex][i].destination] > distance[v.vertex] + adjList[v.vertex][i].weight) continue;
                distance[adjList[v.vertex][i].destination] = distance[v.vertex] + adjList[v.vertex][i].weight;
                parent[adjList[v.vertex][i].destination] = v.vertex;
                minQueue.insert({adjList[v.vertex][i].destination, distance[adjList[v.vertex][i].destination], 0});
            }
        }
